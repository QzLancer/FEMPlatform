                             ...
                             std::vector<Cell *> &omittedCells)
{
  ...
  while(!Q.empty()) {
    ...
    if(s->getBoundarySize() == 1 &&
       ...
       std::abs(s->firstBoundary()->second.get()) < 2) {
      s->getBoundary(bd_s);
      removeCell(s);
  ...
  while(reduced) {
    ...
    while(cit != lastCell(dim - 1)) {
      ...
      if(cell->getCoboundarySize() == 1 &&
         ...
         std::abs(cell->firstCoboundary()->second.get()) < 2) {
        cit++;
        if(dim == omit) {
  ...
  while(reduced) {
    ...
    while(cit != lastCell(dim)) {
      ...
      if(cell->getBoundarySize() == 1 &&
         ...
         std::abs(cell->firstBoundary()->second.get()) < 2) {
        ++cit;
        if(dim - 1 == omit) {
  ...
  for(citer cit = firstCell(dim); cit != lastCell(dim); cit++) {
    ...
    while(Q.size() != 0) {
      ...
      if(s->getCoboundarySize() == 2 && !s->getImmune()) {
        ...
        if(!(*c1 == *c2) && std::abs(or1) == std::abs(or2) && inSameDomain(s, c1) &&
